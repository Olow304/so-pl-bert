...
def main():
    # Load configuration
    config_path = "{data_dir}/config_somali_ft.yml"
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)

    # Override PL-BERT paths to use our Somali model
    config['PLBERT_dir'] = os.path.abspath("runs/plbert_so/packaged")
    config['PLBERT_checkpoint'] = os.path.abspath("runs/plbert_so/packaged/step_000001.pt")

    # Set data paths
    config['train_data'] = os.path.abspath("{data_dir}/train_list.txt")
    config['val_data'] = os.path.abspath("{data_dir}/val_list.txt")
    config['OOD_data'] = os.path.abspath("{data_dir}/OOD_list.txt")
    config['data_path'] = os.path.abspath("{data_dir}/wavs")
    ...
    print(f"PL-BERT model: {{config['PLBERT_dir']}}")
